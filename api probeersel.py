...
data = pd.read_excel('Randon_final_doc.xlsx')
        ...
        try:
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are ChatGPT, a large language model trained by OpenAI. Your task is to assess the provided Python code based on specific criteria and give a score from 1 to 10 for each. Only note the score, no further explanation"
                    },
                    {
                        "role": "user",
                        "content": prompt
...
# Apply the function to each row in the 'codeGPT' column and store the results in the 'Assessment_GPT' column
data['Assessment_GPT'] = data['codeGPT'].apply(assess_code)
data['Assessment_github'] = data['codeGithub'].apply(assess_code)

# Save the dataframe with the new 'Assessment_GPT' column to a new Excel file
data.to_excel("Qualative_assessment_test_part_1.xlsx", index=False)
