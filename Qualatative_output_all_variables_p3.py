...
# Calculate the total count for each metric
github_totals = github_metrics_df.sum()
gpt_totals = gpt_metrics_df.sum()

# Output the dataframe and the total counts
print(df)
print(github_totals)
print(gpt_totals)

# Saving the updated dataframe to a new Excel file
df.to_excel("qualatative_per_metrics_full_v2.xlsx", index=False)
